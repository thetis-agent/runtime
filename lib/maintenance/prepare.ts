...
export async function prepare(root: string, sourceState: string, revision: Revision, capture: Capture): Promise<Result<Prepared>> {
  ...
  try {
    ...
    const state = join(root, 'state'); const copied = await capture(sourceState, state); if (!copied.ok) return copied;
