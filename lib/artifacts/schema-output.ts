...
export async function schemaOutput(root: string, previous?: string): Promise<void> {
  ...
  const documents: Record<string, unknown>[] = []; let bytes = source.value.length;
  for (const directory of [new URL('../../contracts/', import.meta.url), new URL('../../lib/', import.meta.url)]) {
    ...
    for (const entry of entries) {
      ...
      const value: unknown = JSON.parse(document.value.toString('utf8')); if (!isObject(value)) throw new Error('A committed schema is invalid.'); documents.push(value);
    }
  }
