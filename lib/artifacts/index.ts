...
export function sourceFlags(): string[] { return ['--import', fileURLToPath(new URL('./source.mjs', import.meta.url))]; }
export async function compile(source: string, destination: string, previous?: string): Promise<Result<void>> {
  ...
  try {
    return await new Promise(resolve => {
      const child = spawn(process.execPath, [`--max-old-space-size=${String(limits.heapMiB)}`, ...sourceFlags(), fileURLToPath(new URL('./child.ts', import.meta.url))], { env: { PATH: '/usr/bin:/bin' }, stdio: ['pipe', 'pipe', 'pipe'] });
