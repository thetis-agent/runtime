...
class ArtifactError extends Error {
  readonly code: 'outside-roots' | 'budget' | 'hash-mismatch' | 'invalid-args' | 'io';
  constructor(code: ArtifactError['code'], message: string) { super(message); this.code = code; }
}
function read(path: string, maximum: number): Buffer {
...
export function verified(path: string): Buffer {
  const source = read(path, limits.fileBytes);
  const metadata: unknown = JSON.parse(read(`${path}.artifact.json`, limits.metadataBytes).toString('utf8'));
