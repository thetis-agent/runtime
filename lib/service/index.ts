...
export type Factory = (settings: unknown, schemas: Schemas, peer: Peer, identity: ConnectKernel, runToken: string) => Promise<Result<(connection: Connection) => Promise<Result<void>>>>;
export async function serve(factory: Factory, observe: (result: Result<void>) => void, capabilities: readonly string[] = [], scope: 'person' | 'deployment' = 'deployment'): Promise<Result<void>> {
  ...
  try {
    ...
    const handler = await factory(settings.value, schemas, peer, connected.value, inherited.value.token); if (!handler.ok) return handler;
