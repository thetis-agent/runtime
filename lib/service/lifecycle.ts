...
export class Service<C extends string = 'io'> {
  readonly #code: C;
  ...
  #paused = false;
  #closed = false;
  constructor(clock: Clock, limits: typeof serviceLimits, code: C) {
    this.#code = code;
  ...
  open(path: string, serve: (connection: Connection) => Promise<Result<void>>, observe: (outcome: Result<void>) => void): Promise<Result<void, C>> {
    ...
    return new Promise(resolve => {
      this.#server.on('error', () => { this.#closed = true; resolve(failure(this.#code, 'The service socket could not be opened.')); });
  ...
  async #serve(socket: Socket, serve: (connection: Connection) => Promise<Result<void>>, observe: (outcome: Result<void>) => void): Promise<void> {
    ...
    catch { outcome = failure(this.#code, 'The service connection failed.'); }
    finally { probe.abort(); exchange.abort(); socket.destroy(); await probing; if (exchanging) await exchanging; }
    observe(state.expired ? failure('deadline', 'The service connection exceeded its deadline.') : outcome);
  }
  ...
  async resume(): Promise<Result<void>> {
    if (this.#closed) return failure(this.#code, 'The service is already stopped.');
  ...
  async #wait(): Promise<Result<void>> {
    ...
    return drained ? { ok: true, value: undefined } : failure('deadline', 'The service did not drain within its deadline.');
  }
}

export async function listen(path: string, handler: (connection: Connection) => Promise<Result<void>>, observe: (result: Result<void>) => void, clock: Clock, limits = serviceLimits): Promise<Result<Service>> {
  const service = new Service(clock, limits, 'io'); const opened = await service.open(path, handler, observe);
