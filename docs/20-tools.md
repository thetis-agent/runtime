...
| `spawn_subagent` | `task`, `label` (optional) | Creates a child session in the same userspace, sends it the task, and returns `[subagent <id> <label>]` on the first line and the child's final reply after it; without a label the line is `[subagent <id>]`. A stopped child answers `stopped: …` on the second line with what it had said so far, a failed one `error: <message>`; neither is a thrown error. `label` is a short name the person sees, such as `research`. The person watches the child work inside the conversation that spawned it. Stopping the parent turn stops the child. A child may spawn children. See [06-sessions-and-users.md](06-sessions-and-users.md) section 5. |
