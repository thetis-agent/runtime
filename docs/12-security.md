...
- Resources: memory, process count, and CPU per fence through cgroup v2, when the kernel runs in a delegated cgroup. The fence's own group, and only its own, is bound read-only at `/sys/fs/cgroup` so the agent — and any runtime sizing its heap — can read the limit it is held to. It cannot write the group, create a group of its own, or move a process between groups, and it sees no other fence's. See [03-fence.md](03-fence.md) section 3.3.
- Cgroups: the fence has its own cgroup namespace (`--unshare-cgroup`), so the group it runs in is the root of the hierarchy it can see. `/proc/self/cgroup` inside reads `0::/` and names nothing of the host — not the unit, not the slice, not the delegated root, not another fence. Before this, that line carried the host's full path (`/sys/fs/cgroup/system.slice/thetis-runtime.service/fence-<user>`) and told the fence the service's name and where it sits in the host's tree; a fence still learned nothing it could act on, since no other group was mounted, but it could read the layout. Two things are unchanged: `/proc/meminfo`, `/proc/cpuinfo` and the load average are still the host's, and the namespace is isolation of *naming*, not a second limit — the limit is `memory.max` and is enforced by the group itself. On a kernel or bubblewrap without cgroup namespaces (`hasCgroupNamespace` probes for both) the fence falls back to the earlier layout, keeping the host path visible in that one line; nothing else about the fence changes and no fence is refused.
- Mounts: the plan the fence is built from is ordered parents-first and validated, so a mask cannot be silently swallowed by a bind of its parent and two entries cannot quietly claim one path. This is enforcement, not documentation: it is what `fence.hidden` lacked when `$THETIS_HOME` moved under `/opt` and every fence could read the service plane. See [03-fence.md](03-fence.md) section 3.6.
- ssh: a granted key is held by a kernel-owned agent and never enters the fence. The fence can use the credential while it is open and can never copy it; revoking is killing the agent. A grant names one key file, so a fence gets the credential it was given and not the rest of a `~/.ssh`. See [03-fence.md](03-fence.md) section 3.10.
- Docker: nothing, when `fence.docker` is `off`. Otherwise see section 3 — the socket is a deliberate hole and it is host root.
...
- **Docker, when `fence.docker` is not `off`.** This is the largest hole in the fence and it is open by default. The host's Docker socket is bound at `/var/run/docker.sock` in every fence, and a process that can talk to the daemon can ask it for a container with `--privileged` and `/` bound in, so socket access is host root. Everything in section 2 stops applying to what the daemon is asked to run: not the read-only operating system, not the masked `$THETIS_HOME`, not the missing capabilities, not the cgroup limits, not the private network namespace. A fence with Docker is a fence in name only. The trade is accepted for a single-operator installation, where the fence is a tidiness boundary rather than a security one, and it is the wrong trade anywhere a fence holds code that is not already trusted with the host: set `fence.docker` to `"off"` there. A filtering proxy over the daemon API, a daemon per userspace, or a separate build host are the shapes that keep a boundary; none of them is implemented. See [03-fence.md](03-fence.md) section 3.9.
- **Per-package authority.** A fence is one person's authority, not one package's: every package in a fence shares its ssh agent, its mounts, its Docker socket, and — deliberately, see section 4 — every other package's configuration, secrets included. A grant is therefore per person and can never be narrowed to one package. Per-package authority would need a principal per package, which means a fence per package. Until then, installing a package into someone's fence gives it everything that fence holds.
