...
The web gateway accepts `{ input: TurnInput }` at `POST /api/sessions/:id/send`; `{ text }` remains supported. `POST /api/media?name=...` accepts raw bytes with a media type; `GET /api/media/:id` reads an owned asset. These endpoints require the person's authenticated cookie, and writes use the gateway's same-site checks. The transcript renders image/audio/video attachments and download links; unknown parts remain inspectable. The existing composer remains text-only; API clients can send attachments now.
