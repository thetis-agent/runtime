...
The control panel is the part of the web gateway where a person manages packages and, for an admin, people, models, the configuration of every package, mounts, what code each workspace is running, and the installation. It is a place, not a separate address: **Control panel** is the first item of the sidebar's ≡ menu, the panel takes over the main pane, and the close button or the Escape key returns to the conversation. See [15-web-gateway.md](15-web-gateway.md) section 11 for the seam it is built on.
...
| Configuration | `@thetis/ui-admin`, order 32 | Admins | What each package is configured with, and what is missing. One card per installed package from `config-list`, at the system layer, or at one person's own layer from a picker at the top. The card header names the package, what it inherits from, and the kernel's one sentence about it (`every key is set`, `token is required and not set`, `apiKey: OPENROUTER_API_KEY is not in the environment`), red when the package is broken; a line above the cards counts the broken ones. A row per declared key with its help, `required` and `admins only` badges, a control by type (text, number, checkbox, a JSON box for an object or an array, a write-only password box for a secret that says `set` or `not set`), where the value came from (`from the file`, `default`, `set for everyone`, `set by alice`, `inherited from @bitmuse/notion`), and which `${VAR}` is not in the environment. An undeclared key is a row too. **Clear** removes what this layer holds; **Save** sends one `config-set` per key that changed; a key declared `scope: "system"` is read-only in a person's view. **Reload the file** runs `config-reload` and says what changed and which services restarted. See [09-configuration.md](09-configuration.md) section 4. |
...
| Overview | `@thetis/ui-admin`, order 50 | Admins | The configuration file as the kernel reports it, with secrets hidden. Read-only. |
...
| `config` | `config` | none | `config.get`: the configuration file with secrets replaced by `•••`. |
...
| `config-set` | `configSet` | `name`, `key`, `value`, `user?` | `config.set`: writes one key at the system layer, or at the person's with `user`. `value` is any JSON but never null: removing a value is `config-unset`. The value is passed through and never appears in a message. |
| `config-unset` | `configUnset` | `name`, `key`, `user?` | `config.unset`: removes one key from that layer. |
| `config-reload` | `configReload` | none | `config.reload`: re-reads `packages[*]` of `thetis.config.json`; answers `{ changed, restarted }`. |
...
The row itself has no configuration. **Configure** and the sentence about a missing key are on the package's page in the marketplace ([18-marketplace.md](18-marketplace.md) section 9), which the row's **Open in the marketplace** button reaches; the admin's view of every package's configuration is the Configuration section.
...
| `ui-admin/package.json` | The seven `panel` entries and the twenty commands, every one `role: "admin"`. |
| `ui-admin/index.js` | The commands: the checks of section 3, then one operator call each. |
| `ui-admin/ui/index.js` | `install(ext)`: registers the seven sections. |
| `ui-admin/ui/people.js`, `models.js`, `configuration.js`, `mounts.js`, `activity.js`, `workspaces.js`, `overview.js` | One section each, built from `ext.dom` and `ext.ui`, sending through `ext.request`. |
| `ui-admin/ui/config-form.js` | One package's configuration card, from a `ConfigReport`. `@thetis/ui-marketplace` carries a byte-identical copy for the person's own layer, because a package's page may import only its own files; a test in ui-marketplace holds the two together. |
| `ui-admin/ui/index.css` | What the sections add to the shell's styles, under `.ua-`. |

## 7. Tests

`packages/ui-admin/test/ui-admin.test.js` runs each command over a fake `env.kernel.operator.call`: the method and arguments it sends, what it refuses before the kernel is asked, and the own-account refusal. It parses every browser module and checks that the entry defines `install` and nothing else, and that `install` registers exactly the seven declared sections. The `config-*` verbs run over a fake operator call: what they pass to `config.*`, what they refuse, and that nothing is written to the console, so no value can be.

`packages/gateway-web/test/gateway.test.ts` has a third user `root` with role `admin` and `@thetis/ui-admin` in `systemPackages["*"]`. The cases: `api/panel` says `["packages"]` for everyone, `api/ui` lists the seven sections and twenty verbs for `root` and none for a user, and a user gets `403` from a role-admin verb; people are added, changed, and removed through the verbs, and the journal names `root` as the actor; a directory is bound for `bob`, listed, and unbound; a person installs their own package, an admin promotes it through `@thetis/ui-marketplace`'s `promote`, everyone gets it, and an admin installs a shipped package for everyone through `install-everyone`, which a person created afterwards is seeded with; a fork's row says what it replaced, delete with files puts the original back, and a shipped package is refused; `search` and `show` read the index and a README copy, and `models` and `config` answer as the kernel does. `packages/ui-marketplace/test/ui-marketplace.test.js` runs the marketplace commands over a fake environment.
