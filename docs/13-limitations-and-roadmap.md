...
| Component | State |
|---|---|
| Kernel | Config-driven pipeline, built-in provider call, session persistence, the storage contract, the configuration service. 1,386 counted lines of 1,400. |
...
| `/proc/meminfo` reports the host | A fence reads its own limit from the `memory.max` of its own group at `/sys/fs/cgroup` ([03-fence.md](03-fence.md) section 3.3), and a runtime that looks for a cgroup v2 filesystem there — .NET, the JVM — sizes itself from the fence. But `MemTotal` is still the whole machine, so anything that reads it instead — a build tool picking a job count — overshoots the limit and is OOM-killed. | A FUSE layer such as `lxcfs` over `/proc` in the fence. |
...
| No sqlite storage driver | `@thetis/store-toml` is the only driver: one file per document, a directory walk per `list`. Fine for records; not for anything that grows by the thousand. | `@thetis/store-sqlite` on `node:sqlite`: one database, one table, the same conformance suite. See [26-storage.md](26-storage.md) section 5. |
| The journal and the sessions are not on the store | `journal.jsonl` and `userspaces/<user>/sessions/*.json` are still files the kernel writes by path. A second driver would not carry them. | A `Store` for sessions (one document per session; the 256 KiB cap does not apply to the service plane) and an append-only namespace for the journal. |
| A UI command's `env.storage()` is the gateway's | The web gateway hands a command the `StepEnv` its own service started with, so `storage` is bound to `@thetis/gateway-web`, not to the package whose command runs. `env.config` is the command package's, because the gateway fetches it per call. | Bind `storage` per command in `resolveCommand` of `gateway-web/src/ui.ts`, the way the agent's `envFor` does. See [26-storage.md](26-storage.md) section 7. |
| Secrets are plain files | `secrets/*` and `auth/*` are `0600` files under a `0700` directory, not encrypted at rest. Whoever reads the service plane's user reads them. | Acceptable on a single-operator host. An encrypted driver or a keyring behind the same `Store` interface otherwise. |
