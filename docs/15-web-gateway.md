...
| `@thetis/ui-admin` | The admin sections of the control panel: **People**, **Models**, **Mounts**, **Activity**, **Overview**, each a `panel` entry with `role: "admin"` and an `order` that sorts it after the built-in **Packages** (order 10). Twelve commands, every one `role: "admin"`, each a thin wrapper over `env.kernel.operator.call(...)` with the argument checks in front (`users`, `user-create`, `user-role`, `user-status`, `user-password`, `user-remove`, `models`, `config`, `journal`, `mounts-list`, `mounts-set`, `mounts-browse`). A user has the package too and sees none of it: `api/ui` drops the entries and verbs above the role, the gateway answers `403` to a verb sent anyway, and the kernel refuses an operator method from a fence whose user is not an admin. See [17-control-panel.md](17-control-panel.md). |
