...
| `@thetis/ui-marketplace` | The **Marketplace** place: the link in the sidebar footer beside **Control panel**. Opened plain it is the gallery (a search box, one chip per package type, a note on the registries, a card per package with its name, description, version, registry and the badges Only me, Everyone, Available, update to *v*); opened with `{ name }` it is that package's page (the README copy rendered by `ext.markdown`, the facts, what it brings, and the actions the role allows). Eleven commands over the index in `env.shared` and `env.kernel.packages`: `search`, `show`, `install`, `remove`, `delete`, `update` for everyone, and `install-everyone`, `install-for`, `remove-for`, `promote`, `people` with `role: "admin"` over `env.kernel.operator.call`. It depends on `@thetis/marketplace` for `readIndex`, `search`, `readReadme` and `behind`. See [18-marketplace.md](18-marketplace.md) section 9. |
