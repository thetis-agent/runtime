...
| `pkg-fs` | `splitSource`, `isGitSource`, `cloneSlugOf`, `cloneDirFor`, `cloneCommand`, `buildCommand`, `isInside`, `linkDir`, `removeLink`, `keepOnly`, `copyPackageAs`, `findDependency`, `forkOf` (the fork of a package a userspace already holds, read off the installed records: the fork relation backwards, which is the only way to ask it, since only the fork's own manifest names the other end), `forkVersion`, `forkPackage`, `packagesIn`, `packageDigest`, `samePackage`, and the other package file operations. |
