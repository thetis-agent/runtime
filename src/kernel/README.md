...
| `PackageRegistry`, `PackageManager`, `readManifest`, `validateManifest` | Package records, ownership and peer checks, seeding, promotion, install and uninstall, forks and the way back out of one. A clone runs in the receiving userspace's fence, except from a repository the installation holds a key for (a `_system` ssh grant whose `repo` is the same repository): that one is fetched by the system fence, which alone holds the key, and handed to the person as files, so they get the package and never the use of the key. |
