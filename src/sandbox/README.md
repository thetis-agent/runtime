...
The agent gets this environment and nothing else: `PATH`, `HOME`, `LANG`, `THETIS_USERSPACE`, `THETIS_HOME_DIR`, `THETIS_STORE`, `THETIS_SHARED`, `THETIS_USER`, `THETIS_MOUNTS`, `THETIS_HEARTBEAT_MS`, `THETIS_STEP_DEADLINE_MS`, and `THETIS_DOCKER` when a Docker socket is bound. The kernel's own environment does not reach the fence.
