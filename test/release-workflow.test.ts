/** Exercise the exact publication key step without credentials or candidate execution; ADR 0052, ADR 0055. */
...
await test('ADR 0055 both release workflows assemble selected sources and isolate signing from candidate execution', async () => {
  const runtime = await workflow('runtime'); const packages = await workflow('packages');
  assert.equal(keyScript(runtime), keyScript(packages));
  for (const config of [runtime, packages]) {
    const build = steps(config, 'build');
    const action = build.find(step => step['uses'] === './release-tools/actions/verify'); assert.ok(action);
    assert.equal(object(action['with'])['mode'], 'assemble');
...
await test('ADR 0055 raw, transported and base64 release keys produce usable signatures with owner-only permissions', async () => {
  ...
  try {
    ...
    for (const [index, value] of forms.entries()) {
      ...
      const key = join(directory, 'zero-release-key');
...
await test('ADR 0055 invalid and encrypted signing keys fail without printing secrets or retaining a key file', async () => {
  ...
  try {
    ...
    for (const [index, secret] of forms.entries()) {
      ...
      await assert.rejects(stat(join(directory, 'zero-release-key')), /ENOENT/u);
    }
  } finally { await rm(root, { recursive: true, force: true }); }
});
