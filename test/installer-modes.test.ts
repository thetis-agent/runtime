/** Exercise selected provisioning functions and recorded uninstall resources while faking only host-manager commands; ADR 0052. */
...
await test('dry-run follows user, foreground, split and TPM2 provisioning choices', async () => {
  const base = ['--origin', 'https://zero.test', '--operator', 'op', '--dry-run'];
  const user = await install([...base, '--service', 'user', '--no-mount'], false);
  assert.equal(user.status, 0, user.stderr);
  assert.match(user.stdout, /systemctl --user enable --now zero-update.timer/u);
  assert.match(user.stdout, /\.config\/systemd\/user\/zero.service/u);
  assert.doesNotMatch(user.stdout, /useradd|\/etc\/systemd\/system|\/etc\/zero/u);
  const foreground = await install([...base, '--service', 'none', '--no-mount'], false);
  assert.equal(foreground.status, 0, foreground.stderr);
  assert.match(foreground.stdout, /chmod 0600 \/opt\/zero\/etc\/master.key/u);
  ...
  for (const name of ['kernel', 'supervisor', 'g']) assert.match(tpm.stdout, new RegExp(`var-lib-z-${name}\\.mount`, 'u'));
  assert.match(tpm.stdout, /fallocate -l 2147483648 \/var\/lib\/z\/g.img/u);
  assert.doesNotMatch(tpm.stdout, /head -c 32 \/dev\/urandom > \/etc\/zero/u);
});
...
await test('rendered user services omit system identity and mount hardening; TPM2 uses encrypted credentials', async () => {
  ...
  try {
    const harness = '. "$1/defaults.sh"; . "$1/units.sh"; . "$1/provision.sh"; service=$2; key_store=$3; state_layout=split; credential_path=$4; write_unit zero.service "$5"; write_unit zero-update.service "$6"';
    const run = (service: string, key: string, credential: string) => command('/bin/sh', ['-c', harness, 'units', '/workspace/scripts/installer', service, key, credential, join(root, 'zero.service'), join(root, 'update.service')], false);
    const user = await run('user', 'file', '/home/operator/zero/etc/master.key'); assert.equal(user.status, 0, user.stderr);
    const rendered = await readFile(join(root, 'zero.service'), 'utf8');
    ...
    assert.match(rendered, /^LoadCredential=master:\/home\/operator\/zero\/etc\/master.key$/mu);
    assert.doesNotMatch(await readFile(join(root, 'update.service'), 'utf8'), /^(?:User|Group|ProtectHome|ProtectSystem)=/mu);
    const system = await run('system', 'tpm2', '/etc/zero/master.key'); assert.equal(system.status, 0, system.stderr);
    const encrypted = await readFile(join(root, 'zero.service'), 'utf8');
    assert.match(encrypted, /^LoadCredentialEncrypted=master:\/etc\/zero\/master.key$/mu);
    assert.match(encrypted, /^RequiresMountsFor=\/var\/lib\/z \/var\/lib\/z\/kernel \/var\/lib\/z\/supervisor \/var\/lib\/z\/g$/mu);
    assert.match(encrypted, /^User=zero$/mu); assert.doesNotMatch(encrypted, /^LoadCredential=/mu);
...
await test('user uninstall reads its recorded manager, preserves state and key, and makes no writes in dry-run', async () => {
  ...
  try {
    ...
    for (const unit of ['zero.service', 'zero-update.service', 'zero-update.timer']) await writeFile(join(units, unit), 'test unit');
    await writeFile(join(prefix, 'etc/install.json'), JSON.stringify({ version: 1, prefix, state, service: 'user', serviceUser: 'zero',
    ...
    assert.match(commands, /systemctl --user disable --now zero-update.timer zero.service/u);
    ...
    assert.equal(await stat(join(units, 'zero.service')).then(() => true, () => false), false);
  } finally { await rm(root, { recursive: true, force: true }); }
});

await test('installer refuses unsafe paths and unsupported custody combinations before provisioning', async () => {
  for (const prefix of ['/', '/opt/../etc', '/opt/zero;false']) {
    const refused = await install(['--prefix', prefix, '--origin', 'https://zero.test', '--dry-run'], false);
    assert.equal(refused.status, 1, prefix);
  }
  const tpmUser = await install(['--service', 'user', '--no-mount', '--key-store', 'tpm2', '--origin', 'https://zero.test', '--dry-run'], false);
