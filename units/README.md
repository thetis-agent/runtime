...
These four files are the systemd units an installed Zero deployment runs. They are
...
systemd derives from the state path (`/var/lib/z` becomes `var-lib-z.mount`).
...
`zero-update.service` may write only `releases/` and `updates/`, because it
